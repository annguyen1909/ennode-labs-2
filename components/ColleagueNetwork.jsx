...
function NeuralNetworkScene() {
  ...
  useEffect(() => {
    for (let i = 0; i < maxParticleCount; i++) {
      ...
      particlesData.push({
        velocity: vel.normalize().divideScalar(50),
        numConnections: 0,
      });
    }

    // Initially only draw the first `particleCount` points
  ...
  useFrame((_, delta) => {
    ...
    if (linesGeometryRef.current) {
      try {
        ...
        if (linesGeometryRef.current.attributes.position) linesGeometryRef.current.attributes.position.needsUpdate = true;
        if (linesGeometryRef.current.attributes.color) linesGeometryRef.current.attributes.color.needsUpdate = true;
      } catch (e) {}
    }

    if (particlesRef.current && particlesRef.current.attributes && particlesRef.current.attributes.position) {
...
export default function ColleagueNetwork() {
  return (
    <div style={{ width: '100%', height: '100%', minHeight: 520, background: '#060718' }}>
      <Canvas camera={{ position: [0, 0, 30], fov: 50 }} gl={{ antialias: true }}>
        <color attach="background" args={[0x060718]} />
        <NeuralNetworkScene />
      </Canvas>
    </div>
  );
